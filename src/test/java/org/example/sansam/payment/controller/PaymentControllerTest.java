...
class PaymentControllerTest {
    ...
    @Test
    void confirmPayment_INVALID_REQUEST_예외_그대로전파() {
        // given
        TossPaymentRequest request = req(20_000L);
        CustomException ex = new CustomException(ErrorCode.INVALID_REQUEST);
        when(paymentService.confirmPayment(request)).thenThrow(ex);
    ...
    @Test
    void confirmPayment_null_request_예외_그대로전파() {
        // given
        TossPaymentRequest request = null;
        CustomException ex = new CustomException(ErrorCode.INVALID_REQUEST);
        when(paymentService.confirmPayment(null)).thenThrow(ex);

        // when / then
        assertThatThrownBy(() -> controller.confirmPayment(request)).isSameAs(ex);

        verify(paymentService).confirmPayment(null);
        verifyNoMoreInteractions(paymentService);
        verifyNoInteractions(paymentCancelService);
    }

    @Test
    void confirmPayment_예상못한_RuntimeException_그대로전파() {
        // given
        TossPaymentRequest request = req(20_000L);
