...
class PaymentCancelOutBoxServiceTest {
    ...
    @Test
    void enqueue된_레코드의_시간필드_일관성_검증() {
        //given
        service.enqueue("pay_time", 777L, "r", "idem-time");

        //when
        PaymentCancelOutBox o = repo.findAll().get(0);

        // then
        assertThat(o.getUpdatedAt()).isAfterOrEqualTo(o.getCreatedAt());
        long deltaMin = ChronoUnit.MINUTES.between(LocalDateTime.now(), o.getNextRunAt());
        assertThat(Math.abs(deltaMin)).isLessThanOrEqualTo(1);
    }
