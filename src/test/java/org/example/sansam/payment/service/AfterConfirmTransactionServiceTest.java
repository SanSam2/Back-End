...
class AfterConfirmTransactionServiceTest {
    ...
    void 승인성공_새결제저장_주문상태전이_이벤트2건_요청_매핑반환() {
        ...
        // when
        TossPaymentResponse actual = service.approveInTransaction(getManagedOrder(), normalized);
    ...
    void 멱등성_이미해당PaymentKey가_존재하면_저장안하고_상태전이안하고_이벤트발생안하고_기존매핑반환() {
        ...
        // when
        TossPaymentResponse r1 = service.approveInTransaction(getManagedOrder(), normalized);
        TossPaymentResponse r2 = service.approveInTransaction(getManagedOrder(), normalized);
    ...
    void 승인미완료_approvedAtNull이면_결제필수값_누락_에러가_발생한다() {
        ...
        CustomException ex = assertThrows(CustomException.class, () ->
                service.approveInTransaction(getManagedOrder(), normalized)
        );
