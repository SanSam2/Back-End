...
class OrderProductTest {
    ...
    @Test
    void cancelQuantity_초과취소면_예외() {
        //given
        OrderProduct op = newOP(2);

        //when & then
        CustomException ex = assertThrows(CustomException.class,
                () -> op.cancelQuantityCheckChange(5, canceled, partialCanceled));
        assertThat(ex.getErrorCode()).isEqualTo(ErrorCode.CANNOT_CANCEL_MORE_THAN_ORDERED_QUANTITY);
    }

    @Test
    void review작성후_상태값_변경() {
        //given
        OrderProduct op = newOP(2);

        //when
        op.reviewCompletedStatusChange();

        //then
        assertThat(op.getStatus().getStatusName())
                .isEqualTo(StatusEnum.ORDER_PRODUCT_REVIEW_COMPLETED);
    }

    @Test
    void updateOrderProductStatus_정상변경() {
        //given
        OrderProduct op = newOP(1);

        //when
        op.updateOrderProductStatus(paidReviewRequired);

        //then
        assertThat(op.getStatus()).isSameAs(paidReviewRequired);
    }

    @Test
    void updateOrderProductStatus_null이면_예외() {
        //given
        OrderProduct op = newOP(1);

        //when & then
        CustomException ex = assertThrows(CustomException.class,
                () -> op.updateOrderProductStatus(null));
        assertThat(ex.getErrorCode()).isEqualTo(ErrorCode.CHECK_STATUS);
    }
}