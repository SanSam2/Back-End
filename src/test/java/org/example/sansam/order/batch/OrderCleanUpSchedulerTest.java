...
class OrderCleanUpSchedulerTest {
    ...
    @Test
    void cleanUpExpiredOrders_시간이_엣지여도_허용구간_내로_전달된다() {
        // given
        Status waiting = new Status(StatusEnum.ORDER_WAITING);
        given(statusRepository.findByStatusName(StatusEnum.ORDER_WAITING)).willReturn(waiting);
        given(orderRepository.deleteExpiredWaitingOrders(any(), any())).willReturn(1);
        ...
        then(orderRepository).should().deleteExpiredWaitingOrders(eq(waiting), cap.capture());

        // cap된 값 (before-31m) ~ (after-29m) 사이인지 확인
        assertThat(cap.getValue())
                .isAfterOrEqualTo(before.minusMinutes(31))
                .isBeforeOrEqualTo(after.minusMinutes(29));
    }
